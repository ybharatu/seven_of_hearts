...
const playerName = prompt('Please enter your name:'); // Send the player's name to the server 
//socket.emit('setPlayerName', playerName);

//console.log("Going to wait for three players")
socket.emit('setPlayerName', playerName); 
socket.on('updateGameState', (gameState) => 
{ // Update the UI with the new game state 
	console.log('Game state updated:', gameState); 
	cur_gamestate = gameState
	connected_players.textContent = gameState.players.length
	
	// // Delete all existing names
	// // Wait for the DOM to fully load 
	// document.addEventListener('DOMContentLoaded', (event) => {
	// 	all_player_list = document.getElementById('playerList')

	// 	players_tag = all_player_list.querySelectorAll('li');
	// 	console.log("Player List: " + all_player_list)
	// 	console.log(players_tag)
	// 	for(let i = 0; i < players_tag.length; i++){
	// 		all_player_list.removeChild(players_tag[i])
	// 	}
	// })
...
// 		if (cur_gamestate.players.length == 3) { clearInterval(interval); 
// 			resolve(sel_card); } }, 1000); // Check every 1000 milliseconds 
// 			console.log("checking for three players")
// 			console.log(cur_gamestate)
// 		}); 
// }
console.log(cur_gamestate.players.length)
//console.log("Going to wait for three players")
//waitForPlayers();

function waitForThreePlayers() { 
...
let players = [player1, comp1, comp2]
...
async function startGame() {
	...
	//const gameState = await waitForThreePlayers();
	init_states(cur_gamestate)
	await sleep(2000)
	play_init_seven()
	await sleep(5000)
	main_game()
